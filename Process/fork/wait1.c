...
{ 
    ...
    if(cpid == 0){
        ...
        sleep(20);
        printf("Child pid = %d\n", getpid()); 
        exit(1);// we can change this exit status number as well
    } 
    else{    
        printf("\n Parent executing before wait()\n");
        // cpid = wait(NULL); // It won't collect any exit status, We can use when there is no ned to collect child status
        cpid = wait(&status); 
        printf("\n wait() in parent done\nParent pid = %d\n", getpid()); 
        printf("\n cpid returned is (%d)\n",cpid);
        printf("\n status is (%d)\n",status);
...
2. killed by signal
    ...
    unused       |     X   termination signal
                       |
                       |-----> core dump flag
This will be useful when child terminated abnormally
