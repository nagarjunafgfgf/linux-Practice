...
void * threadB(void *p) {
    ...
    pthread_mutex_lock(&my_mutex);
    if (doneFlag == 0)
        pthread_cond_wait(&myConVar, &my_mutex);// unlocks mutex & waits for producer thread_cond signal, Once it receives thread_cond signal again it locks mutex & do the remaining work
